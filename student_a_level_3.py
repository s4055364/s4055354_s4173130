        ...
        rate_change_query = f"""
            WITH formatted_data AS (
                SELECT 
                    ...
                    END as year,
                    -- Extract month (between first and second /)
                    CAST(
                        substr(wd.DMY, 
                            instr(wd.DMY, '/')+1, 
            ...
            date_filtered AS (
                SELECT 
                    site_id,
                    station_name,
                    -- Create comparable date number YYYYMMDD
            ...
            station_averages AS (
                SELECT 
                    ...
                    station_name,
                    AVG(CASE 
                        WHEN date_num BETWEEN 
                            '{start_date1.replace("-", "")}'
                            AND '{end_date1.replace("-", "")}'
                        THEN metric_value 
                    END) as avg_period1,
                    AVG(CASE 
                        WHEN date_num BETWEEN 
                            '{start_date2.replace("-", "")}'
                            AND '{end_date2.replace("-", "")}'
                        THEN metric_value 
                    END) as avg_period2
                FROM date_filtered
                GROUP BY site_id, station_name
                HAVING avg_period1 IS NOT NULL AND avg_period2 IS NOT NULL
            ),
            rates_of_change AS (
                SELECT 
                    site_id,
            ...
            reference_rate AS (
                SELECT percent_change as ref_rate
                FROM rates_of_change
                WHERE site_id = '{reference_station}'
            )
            SELECT 
                r.station_name,
                ROUND(r.avg_period1, 1) as avg_temp_period1,
                ROUND(r.avg_period2, 1) as avg_temp_period2,
                ROUND(r.percent_change, 2) as percent_change,
                ROUND(r.percent_change - rr.ref_rate, 2) as difference_from_ref
            FROM rates_of_change r
            CROSS JOIN reference_rate rr
            WHERE r.site_id != '{reference_station}'
            ORDER BY ABS(r.percent_change - rr.ref_rate)
            LIMIT {num_stations}
            """
        """
        
        
        similar_stations_results = pyhtml.get_results_from_query("database/climate.db", rate_change_query) as avg_temp_period2,
                ROUND(r.percent_change, 2) as percent_change,
                ROUND(r.percent_change - rr.ref_rate, 2) as difference_from_ref
            FROM rates_of_change r
            CROSS JOIN reference_rate rr
